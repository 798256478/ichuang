...
class RecordList extends ComponentBase
{
    ...
    public function onPage(){  
            ...
            $model = new $modelClassName();
            $model = $this->sort($model);  
            if(post('number[1]')=="serve")
            { 
             $diskname=$model->join("system_files",function($join){
                $join->on("acme_serve_shops.id","=","system_files.attachment_id")->where("system_files.attachment_type","LIKE","%Shop");})->select("acme_serve_shops.*","system_files.disk_name")->orderBy("attachment_id","asc")->skip(post('number[0]'))->take(5)->get(); 
            return json_encode($diskname);}else{
                $diskname=$model->join("system_files",function($join){
                $join->on("acme_guide_contents.id","=","system_files.attachment_id")->where("system_files.attachment_type","LIKE","%Content");})->select("acme_guide_contents.*","system_files.disk_name")->orderBy("attachment_id","asc")->skip(post('number[0]'))->take(5)->get(); 
            return json_encode($diskname);
            }
    }
}